...
- **Deep review is owner-invoked.** The runtime-specific skill —
  ...
  runs when the owner invokes it by name, and at no other time. Default scope
  is the branch diff, not the whole repo. The Codex skill fixes confirmed safe
  findings by default; `review-only` must be explicit.
