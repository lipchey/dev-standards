...
export function decideHandoff(findingsFile: FindingsFileV2, deps: HandoffDeps): HandoffResult {
  ...
  const selfReview = findingsFile.self_review;
  if (selfReview === null) {
    reasons.push('no self-review on record (run deep-review self-review after the last slice)');
  } else if (selfReview.verdict !== 'clean') {
    reasons.push(`self-review verdict is ${selfReview.verdict} (resolve the violation and re-run deep-review self-review)`);
  } else if (selfReview.sha !== head) {
    reasons.push(`self-review is stale: reviewed ${selfReview.sha}, HEAD is ${head} (re-run deep-review self-review)`);
  }
