...
задокументований механізм, а не ad-hoc. Сьогодні продумано частково.
...
2. **`.agents/review-guides/`** — проектна адаптація знань: copy+fill сідів
   з `agents/review-guide-templates/` (розв'язання кондиційності
   strong/light/none під тип репо) + capture проектних правил gate-скілами
   (review-memory routing за скоупом).
3. **No-touch floor** — extend-only union: проект може додати зони, звузити
   BASELINE не може.

### Чого нема — додати

1. **Прецедентність і свідомі відхилення (S).** Правило конфлікту гайдів:
   project (`.agents/review-guides/`) > global (`~/.claude/review-guides/`)
   > core-сіди. Свідоме відхилення від core позначається в проектному гайді
   блоком `> deviates-from-core: <причина>`; promotion-сесіям заборонено
   «виправляти» позначені відхилення назад у бік core; конфлікт БЕЗ маркера
   = кандидат на узгодження (у promotions inbox). Зафіксувати в
   `review-output-format.md` або окремому короткому гайді + ADOPTION.md.
2. **Специфікувати `project-facts.md` (S).** Гайди й deep-review вже його
   консумують (layer DAG, домені терміни, no-touch додатки, тип репо для
   кондиційності), але формату/шаблону в ядрі нема. Додати
   `agents/project-facts-template.md`: секції + one-line інструкції
   заповнення; ADOPTION.md крок «заповни project-facts».
3. **Політика скілів: не форкати (S).** Канонічні тіла скілів єдині для всіх
   проектів (ADR-003); проектна специфіка ВИКЛЮЧНО через guides +
   project-facts, які скіли читають. Потрібна поведінка, якої guide-рівень
   не дає → це core-зміна через звичайний fix-upstream цикл (за потреби —
   опційний config-прапор у quality.json), НЕ локальний форк тіла скіла.
   Зафіксувати в CLAUDE.md ядра + ADOPTION.md.

**Acceptance:** ADOPTION.md має розділ «Per-project tuning» з трьома
поверхнями і трьома правилами; `project-facts-template.md` існує; precedence
+ deviation-маркер описані в гайді; у пілоті жоден проектний файл не дублює
тіло core-скіла.

## Порядок і залежності

Фаза 1 → (2, 3, 4 — незалежні, будь-який порядок/паралельно) → 5 → 6, 7
(6 і 7 можна будь-коли після 1; 7 природно робити разом із 6 — обидві
живлять ADOPTION.md; 5.6/5.8 залежать від L2-адопції review-only в
