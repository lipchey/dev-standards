...
2. **Замкнути петлю для пропусків.** Скелет уже існує (promotions inbox +
   package review guides із project-overlay доповненнями у gate-скілах), але
   нема формального сигналу
...
Формальний сигнал пропуску. Міс = дефект, який рання стадія якості
міг/мала зловити, але він дійшов до пізнішої (Gate C, deep-review, рантайм,
юзер-репорт, owner-review). Два роди: детермінований (чек verify мав
зловити) і — з ADR-018 — суддівський (`judgment-missed`: правило належить
review-профілю, і профіль його пропустив).

Формат — append-only рядки (лічильні grep-ом, як inbox):

```text
- [ ] <date> <стадія-де-знайшли> - <one-line дефект> (клас: check-missing |
  too-narrow | report-only-ignored | wrong-tier | judgment-missed) → <маршрут фіксу>
```
...
- `judgment-missed` (маршрут `profile:<name>`, ADR-018) — закривається
  ЛИШЕ з (а) канарейкою в реєстрі профілю
  (`agents/review-guide-templates/TRACEABILITY.md`; канарейки БЛАЙНДЕД —
  ніколи не цитуються в тілах профілів/брифах) і (б) blinded-реплеєм:
  воркер цього профілю на збереженому offending-стані знаходить кейс, не
  знаючи його локації;
