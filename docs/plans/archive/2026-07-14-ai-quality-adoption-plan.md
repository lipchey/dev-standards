...
Status: **historical (implemented)**. N1–N4 completed on 2026-07-14; remaining
work, if any, must have a live owner in `docs/plans/backlog.md`.
...
> Блок 0: ВИКОНАНО 2026-07-14. Gate C-дельта: для 4.2 «лишилась проводка»
> виявилось оверстейтментом — семантика `diff-cover.mjs` розходиться з двома
> контрактними пунктами §4.2 (0%-правило; порожній діапазон); звірку
> зафіксовано в роадмапі, рішення — у low-level плані N3.
> Фаза N4: ВИКОНАНО 2026-07-14 (v0.10.0). Попутний Gate P-улов: e2e-фікстура
> adoption-kit успадковувала реальні `v*`-теги і з появи v0.9.2 інсталювала
> старий снапшот — виправлено герметизацією тег-неймспейсу (синтетичний
> pre-kit v0.8.0 на root-коміті). Формулювальні дельти проти тексту фази:
> Push Protection — НЕ «під локальним gitleaks» (локального тіра нема — шар
...
> Зв'язок з
> `docs/plans/archive/2026-07-10-post-workflow-removal-plan.md`: НЕ замінює
> його. Фази 3-4
